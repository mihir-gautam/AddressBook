...
namespace RestSharpTests
{
    ...
    public class RestSharpTest
    {
        ...
        /// <summary>
        /// UC24 Test method passed when contact address is updated
        /// </summary>
        [TestMethod]
        public void Given_NewAddress_ShouldReturn_UpdatedContact()
        {
            var res = getContactList();
            RestRequest request = new RestRequest("/Contacts/4", Method.PATCH);
            JObject jObjectBody = new JObject();
            jObjectBody.Add("Address", "Mumbai-Nagpur");
            request.AddParameter("application/json", jObjectBody, ParameterType.RequestBody);
            ...
            Assert.AreEqual("Abhijeet", dataResponse.FirstName);
            Assert.AreEqual("Mumbai-Nagpur", dataResponse.Address);
        }
        /// <summary>
        /// UC25 Test method passing when contact of given id is deleted from json
        /// </summary>
        [TestMethod]
        public void GivenId_OnDelete_ShouldReturnSuccessStatusOK()
        {
            var res = getContactList();
            var request = new RestRequest("contacts/3", Method.DELETE);
